...
const MONGODB_URI = 'mongodb+srv://saksham:hellcat007@cluster0.tu5io.mongodb.net/shop';
...
//[single] as we are extracting a single file
...
app.use((req, res, next) => {
    ...
    User.findById(req.session.user._id) //obtaining user from the database
        .then(user => {
            if(!user) //if the user is deleted from the database in between
                return next();
