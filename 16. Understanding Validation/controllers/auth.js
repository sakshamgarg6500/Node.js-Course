    ...
    User.findOne({email: email}) //obtaining user from the database
        .then(user => {
            if(!user) //if the user is not found in the database
            {
                ...
                return res.render('./auth/login',{pageTitle :"Signup", path : '/signup', isAuthenticated: false, errorMessage: 'Invalid Email or Password', oldInput: {email: email, password: password}, validationErrors: []});
            }
            ...
            bcrypt.compare(password, user.password)
                .then(doMatch => { //boolean value will be returned by the [compare] function; [true] if passwords match and [false] if they donot
                    ...
                    req.flash('error', 'Invalid Email or Password');
                    return res.render('./auth/login',{pageTitle :"Signup", path : '/signup', isAuthenticated: false, errorMessage: 'Invalid Email or Password', oldInput: {email: email, password: password}, validationErrors: []}); //if the passwords do not match
                })
                .catch(err => {
                    console.log(err);
                    res.redirect('/login');
